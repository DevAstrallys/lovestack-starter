...
export async function followTicket(params: {
  ticketId: string;
  userId?: string;
  email?: string;
  name?: string;
  ...
  try {
    const { ticketId, userId, email, name, phone } = params;
    if (!userId && !email) {
    ...
    if (phone) row.follower_phone = phone;
    ...
    log.error('Failed to follow ticket', { ticketId: params.ticketId, error: err });
    return false;
  }
}
