-- Create profile for the current user and assign admin role
-- First, insert a profile for the current user
INSERT INTO profiles (id, full_name, locale) 
VALUES (auth.uid(), 'Frédéric LORGEOUX', 'fr')
ON CONFLICT (id) DO UPDATE SET 
  full_name = EXCLUDED.full_name,
  locale = EXCLUDED.locale;

-- Get the admin platform role
WITH admin_role AS (
...
SELECT auth.uid(), admin_role.id, example_org.id, true
